...
flowchart TB
 ...
 subgraph ExceptionHandling["🚨 例外処理<br>Exception Handling"]
    ...
    A -- 注文処理<br>New Order --> B
    A -- 注文キャンセル<br>Cancelled --> A2
    A2 --> Cancelled(["❌ キャンセル<br>Cancelled"])
    Cancelled -.-> V["🔓 Packer 次の作業へ<br>Packer Free to Start Next Task"] & V3["🔓 Inspector 次の作業へ<br>Inspector Free to Process Next Task"]
    B --x PE1
    C --> D
    C --x PE1
    D --> F
